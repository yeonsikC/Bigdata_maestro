...
class WordBook:
    '''
        ...
        method : 생성자( __init__() )
                 단어장 Load( loadWordBook() )
                 단어장 Save( saveWordBook() )
                 단어를 읽는다.( readWord() )
                 단어를 기록한다.( writeWord() )
    ...
    def loadWordBook( self ):
        try:
            with open( 'wordbook.txt', 'r', encoding = 'utf-8' ) as f:
                r = f.readline()
                while r:
                    l = r.split( ',' )
                    self.wordbook[ l[ 0 ] ] = Word( l[ 0 ], l[ 1 ], l[ 2 ] )
                    self.count_word += 1 
                    r = f.readline()

        except FileNotFoundError:
            with open( 'wordbook.txt', 'w', encoding = 'utf-8' ) as f:
                pass
    
    def saveWordBook( self ):
        with open( 'wordbook.txt', 'w', encoding = 'utf-8' ) as f:
            for key in self.wordbook:
                word = self.wordbook[ key ]
                r = '{0},{1},{2},\n'.format( word.getWordName(), word.getWordClass(), word.getWordMeaning() )
                f.write( r )
...
class WordBookUI:
    ...
    def printTitle( self ):
        self.question_str = '''
1. 50개 이내의 단어, 품사, 뜻을 저장한 단어장을 구성한 후
   단어장을 출력하는 프로그램을 class를 이용하여 작성하고
   파일에 단어장을 저장한다.
   ( 단어가 '0'이면 단어 입력 종료 )  
   ( 단어는 최대 20자, 품사는 최대 10자, 뜻은 최대 50자 )
  ...
  2. boundary class와 control class와 Entity class 관계로 구성한다.
    ...
    def printWordBook( self ):
        ...
        for word in self.wordbook.printWordBook():
            print( word )
        print( ''.center( 78, '=' ) )
        print( '총 단어수 : {0:5}\n'.format( self.wordbook.count_word  ) )
...
def main():
    ...
    wordbookUI.printWordBook()
    wordbookUI.wordbook.saveWordBook()    
