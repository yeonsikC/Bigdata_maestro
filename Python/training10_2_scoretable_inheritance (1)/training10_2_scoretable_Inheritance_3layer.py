   ...
   ( 상속과 control/entity class 형태로 구성 )
    ...
    def inputStudentInfo( self ):
        ...
        while  self.name != 'end' and  self.count < ScoreTable.MAX:
            ...
            self.department = input( '전공 [ "computer" ] [ "electronic" ] 중 입력하세요 : ' )
            while ( self.department != 'computer' and self.department != 'electronic' ):
            ...
            if self.department == 'computer':
                for x in range( ComputerStudentInfo.COMPUTER_SUBJECT_MAX ):
                    self.major_subject = int( input( '전공 {0:2} 과목중 {1:2} 번째 전공 과목 성적 입력 : '.format( ComputerStudentInfo.COMPUTER_SUBJECT_MAX, x + 1 ) ) )
                ...
                self.student = ComputerStudentInfo( self.name, self.department, self.common_subjects, self.major_subjects )

            elif self.department == 'electronic':
                for x in range( ElectronicStudentInfo.ELECTRONIC_SUBJECT_MAX ):
                    self.major_subject = int( input( '전공 {0:2} 과목중 {1:2} 번째 전공 과목 성적 입력 : '.format( ElectronicStudentInfo.ELECTRONIC_SUBJECT_MAX, x + 1 ) ) )
                    while ( self.major_subject < 0 or self.major_subject > 100 ):
                        print( '\nError : 점수는 0 ~ 100 입니다...\n다시 입력 하십시요...\n' )
                        self.major_subject = int( input( '전공 {0:2} 과목중 {1:2} 번째 전공 과목 성적 입력 : '.format( ElectronicStudentInfo.ELECTRONIC_SUBJECT_MAX, x + 1 ) ) )    
                    self.major_subjects.append( self.major_subject )
                ...
                for x in range( ElectronicStudentInfo.COMMON_SUBJECT_MAX ):
                    self.common_subject = int( input( '공통 {0:2} 과목중 {1:2} 번째 공통 과목 성적 입력 : '.format( ElectronicStudentInfo.COMMON_SUBJECT_MAX, x + 1 ) ) )
                    while ( self.common_subject < 0 or self.common_subject > 100 ):
                        print( '\nError : 점수는 0 ~ 100 입니다...\n다시 입력 하십시요...\n' )
                        self.common_subject = int( input( '공통 {0:2} 과목중 {1:2} 번째 공통 과목 성적 입력 : '.format( ElectronicStudentInfo.COMMON_SUBJECT_MAX, x + 1 ) ) )
                ...
                self.student = ElectronicStudentInfo( self.name, self.department, self.common_subjects, self.major_subjects )
