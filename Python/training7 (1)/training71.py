...
def calcaverage( students ):
    ...
    '''
    average = sum( students.values() ) / len( students.keys() )
...
def calcmaxmin( students ):
    ...
    for key in keys:
        if maxmininfo[ 'max' ][ 1 ] < students[ key ]:
            maxmininfo[ 'max' ][ 0 ] = key
            maxmininfo[ 'max' ][ 1 ] = students[ key ]
        if maxmininfo[ 'min' ][ 1 ] > students[ key ]:
            maxmininfo[ 'min' ][ 0 ] = key
            maxmininfo[ 'min' ][ 1 ] = students[ key ]
